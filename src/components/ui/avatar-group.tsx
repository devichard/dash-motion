...
function AvatarMotionContainer({ children, zIndex, tooltipContent, tooltipProps }: AvatarMotionProps) {
  return (
...
type AvatarGroupProps = Omit<React.ComponentProps<"div">, "translate" | "transition"> & {
...
function AvatarGroup({
  ...
  if (variant === "stack") {
    return (
      <div ref={ref} className={cn("-space-x-1 flex items-center", className)} {...props}>
        {Children.map(children, (child, index) => {
          if (!child) {
            return null;
          }
          return (
            // biome-ignore lint/suspicious/noArrayIndexKey: Children order is stable in this component
      ...
      >
        {children?.map((child, index) => {
          ...
          if (variant === "motion") {
            return (
              // biome-ignore lint/suspicious/noArrayIndexKey: Children order is stable in this component
              <AvatarMotionContainer key={index} zIndex={zIndex} tooltipProps={tooltipProps}>
                {child}
